...
public enum NodeType { Negation, Universal, Existential, Conjunction, Disjunction, Implication, Biconditional, Variable, Predicate }
...
public struct ASTNode {
    ...
    ASTNode* left;
    ASTNode* right;
    ASTNode*[] args; // for predicates

    public string toString() const {
        if (type == NodeType.Variable) {
            return format("ASTNode[ type: %s, value: %s ]", type, value);
        } else if (type == NodeType.Predicate) {
            string argsStr;
            foreach (arg; args) {
